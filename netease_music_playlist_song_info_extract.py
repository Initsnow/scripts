...
def get_playlist_data(playlist_id):
    ...
    while True:
        data = apis.playlist.GetPlaylistAllTracks(playlist_id, offset=offset, limit=limit)["songs"]
        if not data:
            break
        for track in data:
            name = track["name"]
...
def copy_all_to_clipboard(playlist_data):
    """将所有歌曲信息格式化并复制到剪贴板"""
    formatted_info = "\n".join(
        [f"歌名: {song['name']}; 艺术家: {song['artist']}; 专辑: {song['album']}" for song in playlist_data]
    )
    pyperclip.copy(formatted_info)
    print("已复制所有歌曲信息到剪贴板。")
...
if __name__ == "__main__": 
    ...
    for i, song in enumerate(playlist_data, start=1):
        print(f"{i}. 歌名: {song['name']}; 艺术家: {song['artist']}; 专辑: {song['album']}")
